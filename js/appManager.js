...
(function (CloudExperienceHost) {
    class AppManager {
        ...
        initialize(args) {
            ...
            if (CloudExperienceHost.FeatureStaging.isOobeFeatureEnabled("CxhExtensionPackagesAccess")) {
                // Add all available undocked packages to the wwahost.exe process package graph
                // This is best-effort and depends on an object that may not exist for all editions with CXH, so wrap it in a try-catch
                try {
                    CloudExperienceHostAPI.AppExtensionsManager.addExtensionPackagesToProcessPackageGraph();
                    CloudExperienceHost.Telemetry.AppTelemetry.getInstance().logEvent("AddExtensionPackagesToProcessPackageGraphSucceeded");
                }
                catch (ex) {
                    CloudExperienceHost.Telemetry.AppTelemetry.getInstance().logEvent("AddExtensionPackagesToProcessPackageGraphFailed", CloudExperienceHost.GetJsonFromError(ex));
                }
            }
        ...
        _getContext() {
            if (this._description) {
                ...
                if (CloudExperienceHost.FeatureStaging.isOobeFeatureEnabled("OobeOfficeAsksFY22H1")) {
                    ...
                    context.capabilities = JSON.stringify({
                        "PrivatePropertyBag": 1, "PasswordlessConnect": 1, "Inclusive": inclusive, "IsCloudPolicyEnforced": isCloudPolicyEnforced, "PasswordlessSelfConnect": 1
                    });
                }
        ...
        _onNavigationStarting(node) {
            ...
            if (CloudExperienceHost.FeatureStaging.isOobeFeatureEnabled("AllowDisplayModeChange")) {
        ...
        _onNavigationCompleted(node) {
            ...
            this._pendingNode = null;
            let isOobeOfficeAsksFY22H1Enabled = CloudExperienceHost.FeatureStaging.isOobeFeatureEnabled("OobeOfficeAsksFY22H1");
            let isCBREnabled = CloudExperienceHost.FeatureStaging.isOobeFeatureEnabled("CBR");
            if ((this._currentNode) && (this._currentNode.cxid === node.cxid)) {
                if (isOobeOfficeAsksFY22H1Enabled || isCBREnabled) {
