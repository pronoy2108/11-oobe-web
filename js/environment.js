...
(function (CloudExperienceHost) {
    class Environment {
        ...
        static GetWiFiHostedApplicationArgumentsDesktop() {
            ...
            if (CloudExperienceHostAPI.FeatureStaging.isOobeFeatureEnabled("DesktopLiteOobe")) {
    ...
    class OobeContextHelper {
        static isLiteOobeApplicableRuntimeCheck(featureName) {
            return !CloudExperienceHostAPI.UtilStaticsCore.isAssignedToLiteMetricExperimentGroup ? CloudExperienceHost.AccountDisambiguation.isLiteOobeApplicableForAadCapableEditions(featureName) : false;
        }
        static isFullScreenAndFeatureEnabled(featureName) {
            return FeatureStaging.isOobeFeatureEnabled(featureName) && CloudExperienceHostAPI.Environment.isFullscreenCXHRunning;
        }
    }
    CloudExperienceHost.OobeContextHelper = OobeContextHelper;
    ...
    class NetworkLossAversionV2 {
        // skip v2 when active personality is lite white
        static getShouldSkipAsync() {
            return WinJS.Promise.wrap(CloudExperienceHost.getNavMesh().getPersonality() === CloudExperienceHost.TargetPersonality.LiteWhite);
        }
    }
    CloudExperienceHost.NetworkLossAversionV2 = NetworkLossAversionV2;
    class NetworkLossAversionV3 {
        // skip v3 when active personality is blue
        static getShouldSkipAsync() {
            return WinJS.Promise.wrap(CloudExperienceHost.getNavMesh().getPersonality() !== CloudExperienceHost.TargetPersonality.LiteWhite);
        }
    }
    CloudExperienceHost.NetworkLossAversionV3 = NetworkLossAversionV3;
    ...
    class AccountDisambiguation {
        ...
        static getLiteOobeGatingFeatureForAadCapableEditions() {
            return (CloudExperienceHost.getAllowedIdentityProviders().indexOf(CloudExperienceHost.SignInIdentityProviders.AAD) != -1) ? "AadCapableDLOOBE" : null;
        }
        static isLiteOobeApplicableForAadCapableEditions(featureName) {
            let applicableFeature = AccountDisambiguation.getLiteOobeGatingFeatureForAadCapableEditions();
            let isFeatureApplicable = applicableFeature ? FeatureStaging.isOobeFeatureEnabled(applicableFeature) : true;
            return isFeatureApplicable ? FeatureStaging.isOobeFeatureEnabled(featureName) : false;
        }
    }
        ...
        class Eligibility {
            ...
            static shouldRestrictionsApplyToCurrentUserAsync(shouldRestrictionsApplyToMinorOverStatutoryAge = false) {
                return new WinJS.Promise((completeDispatch /*, errorDispatch, progressDispatch */) => {
                    if (CloudExperienceHostAPI.FeatureStaging.isOobeFeatureEnabled("CxhAadcBehavior")) {
